...
impl State {
    ...
    fn game_over(&mut self, ctx: &mut BTerm) {
        ...
        ctx.print_color_centered(4, WHITE, BLACK, "Slain by a monster, yout heroe's journey has come to a premature end");
    ...
    fn victory(&mut self, ctx: &mut BTerm) {
        ...
        ctx.print_color_centered(4, WHITE, BLACK, "Your town is saved and you can return to your normal life");
        ctx.print_color_centered(9, GREEN, BLACK, "Press 1 to play again");
        if let Some(VirtualKeyCode::Key1) = ctx.key {
            self.reset_game_state();
        }
    ...
    fn advance_level(&mut self) {
        ...
        <(Entity, &Carried)>::query()
            .iter(&self.ecs)
            .filter(|( _e, carried )| carried.0 == player_entity)
            .map(|(e, _carried)| *e)
